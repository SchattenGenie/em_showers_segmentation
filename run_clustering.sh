...
min_samples_core=${min_samples_core:-4}
vanilla_hdbscan=${vanilla_hdbscan:-False}
while [ $# -gt 0 ]; do
   if [[ $1 == *"--"* ]]; then
        param="${1/--/}"
        declare $param="$2"
...
--project_name em_showers_network_clustering_new --workspace SchattenGenie \
...
--cl_size $cl_size --min_cl $cl_size --threshold $threshold --min_samples_core $min_samples_core \
--vanilla_hdbscan $vanilla_hdbscan